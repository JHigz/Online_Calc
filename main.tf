...
resource "aws_instance" "cyber94_jhiguita_calculator_2_webserver_tf" {
    ...
    vpc_security_group_ids = [aws_security_group.cyber94_jhiguita_calculator_2_webserver_sg_tf_.id]
    user_data = templatefile("../init-scripts/docker-install.sh", {SPECIAL _ARG="This is an argument from Terraform"})
    tags = {
      Name = "cyber94_jhiguita_calculator_2_webserver_${count.index}"
    }
}
